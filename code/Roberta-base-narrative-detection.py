...
# testing_set = NarrativesData(test_data, tokenizer, MAX_LEN)

training_loader = DataLoader(training_set, **train_params)
validation_loader = DataLoader(validation_set, **val_params)
# testing_loader = DataLoader(testing_set, **val_params)
# print(len((valid_data['label'].unique())))
...
for epoch in range(EPOCHS):
    train(epoch)    
torch.save(model.state_dict(),'roberta_models/roberta-base_%s_dropout0.3'%LEARNING_RATE )
...
# model.load_state_dict(torch.load('roberta_models/roberta-base_2e-5_dropout0.1', map_location='cpu', weights_only=True))
